    ...
    for x in steam_id_list:
        ...
        if out is not None:
            if len(out.body.schema) > 0:
                with open(
                    os.path.join(
                        backup_directory, "UserGameStatsSchema_{}.bin".format(game_id)
                    ),
                    "wb",
                ) as f:
    ...
    if len(file_details.file_url) > 0:
        with urllib.request.urlopen(file_details.file_url) as response:
            ...
            with open(
                os.path.join(
                    backup_directory,
                    file_details.filename.replace("/", "_").replace("\\", "_"),
                ),
                "wb",
            ) as f:
                f.write(data)
        ...
        print(
            ...
            published_file_id,
            "no url (you can ignore this if the game doesn't need a controller config)",
        )
        return None
...
def generate_inventory(client, game_id):
    ...
    url = "https://api.steampowered.com/IGameInventory/GetItemDefArchive/v0001?appid={}&digest={}".format(
        game_id, inventory.body.digest
    )
    try:
        with urllib.request.urlopen(url) as response:
...
def get_dlc(raw_infos):
    try:
        try:
            dlc_list = set(
                map(lambda a: int(a), raw_infos["extended"]["listofdlc"].split(","))
            )
        except:
            dlc_list = set()
        depot_app_list = set()
...
def generate_config(appid, username, password):
    appids = [appid]
        ...
        while result in (
            ...
            if result == EResult.InvalidPassword:
                ...
                prompt = (
                    ...
                    else "Incorrect code. Enter email code: "
                )
                ...
                prompt = (
                    ...
                    else "Incorrect code. Enter 2FA code: "
                )
        ...
        if "common" in game_info:
            ...
            if "supported_languages" in game_info_common:
                with open(os.path.join(out_dir, "supported_languages.txt"), "w") as f:
                    ...
                    for l in languages:
                        if (
                            "supported" in languages[l]
                            and languages[l]["supported"] == "true"
                        ):
                            f.write("{}\n".format(l))
        ...
        if "config" in game_info:
            if "steamcontrollerconfigdetails" in game_info["config"]:
                ...
                for id in controller_details:
                    ...
                    out_vdf = download_published_file(
                        client,
                        int(id),
                        os.path.join(backup_dir, controller_type + str(id)),
                    )
                    if out_vdf is not None and not config_generated:
                        if controller_type in [
                            ...
                            ("default" in enabled_branches)
                            or ("public" in enabled_branches)
                        ):
                            parse_controller_vdf.generate_controller_config(
                                out_vdf.decode("utf-8"),
                                os.path.join(out_dir, "controller"),
                            )
                            config_generated = True
            if "steamcontrollertouchconfigdetails" in game_info["config"]:
                controller_details = game_info["config"][
